...
local function getDeathState(src)
	...
	if not player then return sharedConfig.deathState.ALIVE end
...
---@param player table|number accepts a server id or a qbx player object
local function revivePlayer(player)
    if type(player) == 'table' then
        player = player.PlayerData and player.PlayerData.source
    end
    if type(player) ~= 'number' then return false end
    TriggerClientEvent('qbx_medical:client:playerRevived', player)
    return true
end
