...
---Validates a client-supplied hospital name + bed index against the configured
---bed map. Without this, a forged event with an unknown hospital name throws a
---nil-index error, and out-of-range indices corrupt occupancy tracking.
---@return boolean
local function isValidBed(hospitalName, bedIndex)
	local beds = hospitalName and hospitalBedsTaken[hospitalName]
	if not beds then return false end
	bedIndex = tonumber(bedIndex)
	if not bedIndex or bedIndex < 1 or bedIndex > #beds then return false end
	return true
end
...
RegisterNetEvent('qbx_ambulancejob:server:playerEnteredBed', function(hospitalName, bedIndex)
	...
	local src = source
	if not isValidBed(hospitalName, bedIndex) then return end
	billPlayer(src)
...
local function checkIn(src, patientSrc, hospitalName)
	...
	-- Reserve the bed immediately so two concurrent check-ins can't be handed
	-- the same index. The client `playerEnteredBed` callback re-sets it (no-op)
	-- and `playerLeftBed` releases it.
	hospitalBedsTaken[hospitalName][bedIndex] = true
...
local function respawn(src)
	...
	hospitalBedsTaken[closestHospital][bedIndex] = true
...
-- Player-driven "respawn at hospital". Must only be honored for players who are
-- actually dead or in last-stand, otherwise any client can trigger a free
-- teleport to a hospital and (when wipeInvOnRespawn is set) an inventory wipe.
RegisterNetEvent('w2f-ambulance:server:forceHospitalRespawn', function()
	...
	if not (meta and (meta.isdead or meta.inlaststand)) then return end
	TriggerEvent('qbx_medical:server:playerRespawned', src)
end)
