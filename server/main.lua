...
local function onPlayerDiedAlert()
	if GetInvokingResource() then return end
	local src = source
	alertAmbulance(src, locale('info.civ_died'))
end

-- `onPlayerDied` is the name the death client actually emits
-- (medical/client/dead.lua). `playerDied` is kept for external/legacy
-- qbx_medical compatibility. Previously only `playerDied` was registered, so
-- death alerts never fired.
RegisterNetEvent('qbx_medical:server:onPlayerDied', onPlayerDiedAlert)
RegisterNetEvent('qbx_medical:server:playerDied', onPlayerDiedAlert)