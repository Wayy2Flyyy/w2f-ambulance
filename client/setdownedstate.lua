...
local function displayRespawnText()
    local deathTime = W2FAmbulance.Medical('GetDeathTime')
    if deathTime > 0 and doctorCount > 0 then
        ...
        qbx.drawText2d({
            text = locale('info.respawn_revive', W2FAmbulance.Medical('GetRespawnHoldTimeDeprecated'), sharedConfig.checkInCost),
...
local function playDeadAnimation(ped)
    if IsInHospitalBed then
        ...
        W2FAmbulance.Medical('PlayDeadAnimation')
    end
end
...
local function handleLastStand()
    local laststandTime = W2FAmbulance.Medical('GetLaststandTime')
...
CreateThread(function()
    ...
    while true do
        local isDead = W2FAmbulance.Medical('IsDead')
        local inLaststand = W2FAmbulance.Medical('IsLaststand')
