...
local function mapTreatmentLabel(required)
    local map = { treat = 'Treat Wounds', repair = 'Stabilize Before Transport', help = 'Assist Breathing', revive = 'CPR / Revive Patient', none = 'No Treatment Required' }
    return map[required] or 'Further Assessment Required'
end

function Radial.RunPatientTreatment(actionType, eventName)
    if not activePatientStatus or not activePatientStatus.active then
        SendNUIMessage({ action = 'radial:patientStatusError', data = { kind = 'treatment', message = 'No active patient assessment. Run a scan first.' } })
        return
    end
    if eventName then
        TriggerEvent(eventName)
    end
    if actionType == 'rx' or actionType == 'load' or actionType == 'transport' then
        CreateThread(function()
            Wait(900)
            TriggerEvent('w2f-ambulance:client:patientTreatmentResult', { success = true, action = actionType, message = 'Treatment completed.' })
        end)
    end
end
...
function Radial.Register()
    ...
    function Radial.CheckPatientStatus()
        ...
        if not player then
            SendNUIMessage({ action = 'radial:patientStatusError', data = { kind = 'scan', message = 'No Patient Found. Move closer and retry.' } })
            return
        end
        ...
        if not result or not result.ok then
            SendNUIMessage({ action = 'radial:patientStatusError', data = { kind = 'scan', message = result and result.message or 'Unable To Scan. Move closer and retry.' } })
            return
        end
    ...
    bind('ems_ps_treat', bindCare('TreatWounds', function() Radial.RunPatientTreatment('treat', 'hospital:client:TreatWounds') end))
    bind('ems_ps_bandage', bindCare('TreatWounds', function() Radial.RunPatientTreatment('treat', 'hospital:client:TreatWounds') end))
    bind('ems_ps_rx', bindCare('IssuePrescription', function() Radial.RunPatientTreatment('rx', 'w2f-ambulance:client:issuePrescriptionNearest') end))
    bind('ems_ps_stabilize', bindCare('TreatWounds', function() Radial.RunPatientTreatment('repair', 'hospital:client:StabilizePatient') end))
    bind('ems_ps_ifak', bindCare('TreatWounds', function() Radial.RunPatientTreatment('repair', 'hospital:client:StabilizePatient') end))
    bind('ems_ps_assist', bindCare('TreatWounds', function() Radial.RunPatientTreatment('help', 'hospital:client:AssistPatient') end))
    bind('ems_ps_support', bindCare('TreatWounds', function() Radial.RunPatientTreatment('help', 'hospital:client:AssistPatient') end))
    bind('ems_ps_cpr', bindCare('RevivePatient', function() Radial.RunPatientTreatment('revive', 'hospital:client:RevivePlayer') end))
    bind('ems_ps_revive', bindCare('RevivePatient', function() Radial.RunPatientTreatment('revive', 'hospital:client:RevivePlayer') end))
    bind('ems_ps_stabilize_revive', bindCare('TreatWounds', function() Radial.RunPatientTreatment('repair', 'hospital:client:StabilizePatient') end))
    bind('ems_ps_load', bindCare('PutPatientInBed', function()
        if W2FAmbulance.HospitalClient and W2FAmbulance.HospitalClient.putNearestPatientInBed then
            Radial.RunPatientTreatment('load')
            W2FAmbulance.HospitalClient.putNearestPatientInBed()
        else
            exports.qbx_core:Notify('Transport loading is not configured yet.', 'error')
            Radial.OnTreatmentComplete({ success = false, reason = 'Transport loading is not configured yet.' })
        end
    end))
    bind('ems_ps_transport', bindCare('PutPatientInBed', function()
        if W2FAmbulance.HospitalClient and W2FAmbulance.HospitalClient.putNearestPatientInBed then
            Radial.RunPatientTreatment('transport')
            W2FAmbulance.HospitalClient.putNearestPatientInBed()
        else
            exports.qbx_core:Notify('Transport workflow is not configured yet.', 'error')
            Radial.OnTreatmentComplete({ success = false, reason = 'Transport workflow is not configured yet.' })
        end
    end))
    bind('ems_ps_back', function()
        Radial.ClearPatientStatus()
        UI.Radial.Navigate('ems_patient', true)
    end)
